...
/* FSM system instance. */
static FSMSystem sys(AUTOEXAMPLE_FSM + 1 /* max number of automates types */, AUTOEXAMPLE_MBX_ID + 1 /* max number of msg boxes */);
...
DWORD WINAPI SystemThread(void *data) {
	...
	sys.Add(&automate[1], AUTOEXAMPLE_FSM);//, 3 /* the number of automates that will be added */, true);
	sys.Add(&automate[2], AUTOEXAMPLE_FSM);//, 3 /* the number of automates that will be added */, true);
